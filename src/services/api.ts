...
const Fetcher = async (uri: string, method: string = 'GET', object_data: {} = {}) => {
    ...
    try {
        ...
        try {
            ...
            if (error.toString().substring(0, 29) === "SyntaxError: Unexpected token" || error.toString() === 'SyntaxError: Unexpected end of JSON input') return { message: res?.statusText, status: res?.status };
